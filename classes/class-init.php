...
class Init {
	 ...
	 */
	private string $definition_path = 'data/project-definition.json';
	...
	public function setup() {
		...
		if ( ! is_array( $this->def  ) || ! array_key_exists( 'key', $this->def ) ) {
			error_log( 'BigupWeb\CPT_Project error: Could not retrieve post type definition' );
			return;
		}
	...
	private function get_definition() {
		$json       = Util::get_contents( CPTPRO_DIR . $this->definition_path );
	...
	public function enqueue_editor_scripts() {
		wp_register_script( 'bigup_cpt_project_js', CPTPRO_URL . 'build/metaboxPlugin.js', array(), filemtime( CPTPRO_DIR . 'build/metaboxPlugin.js' ), true );
		wp_enqueue_script( 'bigup_cpt_project_js' );
	}
}
